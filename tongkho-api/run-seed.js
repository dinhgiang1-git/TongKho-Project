...
async function main() {
  const connection = await mysql.createConnection({
    ...
    password: '1211',
    database: 'tongkho_db',
    multipleStatements: true // Bật tính năng chạy nhiều câu lệnh
  });
  ...
  try {
    ...
    `);
    console.log('Đã thêm dữ liệu Category');
    ...
    `);
    console.log('Đã thêm dữ liệu Supplier');
    ...
    `);
    console.log('Đã thêm dữ liệu Warehouse');
    ...
    `);
    console.log('Đã thêm dữ liệu Product');

    console.log('HOÀN TẤT!');
  } catch (err) {
    console.error('LỖI THÊM DỮ LIỆU:', err.message);
  } finally {
